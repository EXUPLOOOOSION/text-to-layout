...
# Training
STARTING_EPOCH = 0 #epoch to start  training back from. The STARTING_EPOCH th model, ie. model with index STARTING_EPOCH-1 will be loaded from CHECKPOINTS_PATH. If STARTING_EPOCH<=0 no epoch will be loaded.
...
CHECKPOINTS_PATH = "./checkpoints/unfrozen_128" # Path to save the epochs and average losses
LEARNING_RATE = 5e-5
...
VALIDATION_OUTPUT = "./evaluator_output/unfrozen_128" # Path to save the output (bbox and class for each picture)
...
EPOCHS_VALIDATION = [0]#list(range(STARTING_EPOCH,STARTING_EPOCH+EPOCHS))#list(range(0,EPOCHS,3)) # Number of the epoch to validate
...
def generate_losses(vocab, train_ds):
    ...
    # bbox loss
    bloss_xy = nn.CrossEntropyLoss(ignore_index=0)
    bloss_wh = bbox_loss
