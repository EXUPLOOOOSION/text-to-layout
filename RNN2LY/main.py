...
#encoder-decoder
CONFIG_FILE_PATH = './checkpoints/carlos_pt/model_config.json'
...
CHECKPOINTS_PATH = "./checkpoints/carlos_pt" # Path to save the epochs and average losses
SAVE_LANG = True
LANG_PATH = './checkpoints/langs/kek.pickle'
LEARNING_RATE = 5e-5
...
VALIDATION_OUTPUT = "./evaluator_output/carlos_pt" # Path to save the output (bbox and class for each picture)
...
EPOCHS_VALIDATION = [27]#list(range(0,EPOCHS,3)) # Number of the epoch to validate
...
def generate_dataset(dataset_path_train, instan_path_train, dataset_path_test, instan_path_test, 
    ...
    print("Saving Lang in "+LANG_PATH)
    if SAVE_LANG:
        try:
            with open(LANG_PATH, 'wb') as f:
                pickle.dump(train_ds.cap_lang, f)
        except Exception as e:
            print("Error while trying to save lang")
