...
def generate_encoder(freeze_encoder=False):
    """
...
def generate_decoder(vocab, is_training, use_attention=False, bidirectional=True, hidden_size=128, xy_distribution_size=16, temperature=0.4):
    """
...
def load_config(configuration_file_path):
    ...
    "temperature": temperature to use in tempearture softmax for xy coordinate sampling.
    ...
    try:
        ...
        traceback.print_exc()
        exit()
    encoder, decoder = generate_encoder(configuration['freeze_encoder']), generate_decoder(configuration['num_classes'], False, xy_distribution_size=configuration['xy_distribution_size'], use_attention=configuration['use_attention'], hidden_size=configuration['hidden_size'],temperature=configuration['temperature'])
